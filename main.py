...
class MetabolicNetwork:
    ...
    def pygame_draw_compartment_legends(self, display):
        # compartment legends
        for idx, compartment in enumerate(self.compartments):
    ...
    def pygame_draw_lines(self, display):
        if self.show_lines:
            for idx, node in enumerate(self.all_network_nodes):
                if node.visible_by_mouse:
    ...
    def pygame_search_mode(self, display, text_input_rect):
    ...
    def pygame_show_expression_or_fluxes(self, display):
    ...
    def pygame_event_handling_code(self, display):
        global running
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.search_mode_enabled:
                    ...
                    self.key_actions["v_clicked"] = True
                elif event.key == pygame.K_x:
                    self.key_actions["split_clicked"] = True
                elif event.key == pygame.K_UP or event.key == pygame.K_w:
                    self.key_actions["up_clicked"] = True
                elif event.key == pygame.K_DOWN or event.key == pygame.K_s:
                    self.key_actions["down_clicked"] = True
                elif event.key == pygame.K_LEFT or event.key == pygame.K_a:
                    self.key_actions["left_clicked"] = True
                elif event.key == pygame.K_RIGHT or event.key == pygame.K_d:
                    self.key_actions["right_clicked"] = True
                elif event.key == pygame.K_f:
                    self.key_actions["f_clicked"] = True
                    if self.key_actions["shift_clicked"] or self.key_actions["control_clicked"]:
                        set_to_freeze = False
                        for node in self.all_network_nodes:
                ...
                elif event.key == pygame.K_z and self.key_actions["control_clicked"]:
                    self.undo_last_action()

                elif event.key == pygame.K_y and self.key_actions["control_clicked"]:
                    self.redo_last_action()
            elif event.type == pygame.KEYUP:
                if event.key == pygame.K_LCTRL:
                    self.key_actions["control_clicked"] = False
                elif self.search_mode_enabled:
                    pass
                elif event.key == pygame.K_LSHIFT:
                    self.key_actions["shift_clicked"] = False
                elif event.key == pygame.K_v:
                    self.key_actions["v_clicked"] = False
                elif event.key == pygame.K_x:
                    self.key_actions["split_clicked"] = False
                elif event.key == pygame.K_UP or event.key == pygame.K_w:
                    self.key_actions["up_clicked"] = False
                elif event.key == pygame.K_DOWN or event.key == pygame.K_s:
                    self.key_actions["down_clicked"] = False
                elif event.key == pygame.K_LEFT or event.key == pygame.K_a:
                    self.key_actions["left_clicked"] = False
                elif event.key == pygame.K_RIGHT or event.key == pygame.K_d:
                    self.key_actions["right_clicked"] = False
                elif event.key == pygame.K_f:
                    self.key_actions["f_clicked"] = False


            elif event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and self.key_actions["split_clicked"]:
                    for node in self.all_network_nodes:
                ...
                elif event.button == 1 and self.key_actions["shift_clicked"]:
                    mouse_pos = pygame.mouse.get_pos()
                    scaled_mouse_x = (mouse_pos[0] + self.viewport.x) / self.zoom_factor
                    scaled_mouse_y = (mouse_pos[1] + self.viewport.y) / self.zoom_factor
                    mouse_pos = (scaled_mouse_x, scaled_mouse_y)
                ...
                elif event.button == 1 and self.key_actions["v_clicked"]:
                    mouse_pos = pygame.mouse.get_pos()
                    scaled_mouse_x = (mouse_pos[0] + self.viewport.x) / self.zoom_factor
                    scaled_mouse_y = (mouse_pos[1] + self.viewport.y) / self.zoom_factor
                    mouse_pos = (scaled_mouse_x, scaled_mouse_y)
                ...
                elif event.button == 1 and self.key_actions["f_clicked"]:
                    self.key_actions["right_mouse_clicked"] = True
                    mouse_pos = pygame.mouse.get_pos()
                    scaled_mouse_x = (mouse_pos[0] + self.viewport.x) / self.zoom_factor
                    scaled_mouse_y = (mouse_pos[1] + self.viewport.y) / self.zoom_factor
                    mouse_pos = (scaled_mouse_x, scaled_mouse_y)
                    ...
                    self.key_actions["left_mouse_clicked"] = True
                    mouse_pos = pygame.mouse.get_pos()
                    scaled_mouse_x = (mouse_pos[0] + self.viewport.x) / self.zoom_factor
                    scaled_mouse_y = (mouse_pos[1] + self.viewport.y) / self.zoom_factor
                    mouse_pos = (scaled_mouse_x, scaled_mouse_y)
                    ...
                    self.key_actions["middle_mouse_clicked"] = True
                    mouse_pos = pygame.mouse.get_pos()
                    scaled_mouse_x = (mouse_pos[0] + self.viewport.x) / self.zoom_factor
                    scaled_mouse_y = (mouse_pos[1] + self.viewport.y) / self.zoom_factor
                    mouse_pos = (scaled_mouse_x, scaled_mouse_y)
                ...
                if event.button == 1 and not self.key_actions["shift_clicked"]:
                    self.key_actions["left_mouse_clicked"] = False
                    self.selected_node = None
                    self.start_command_recording = True
                    if self.drawing_selection_rect_on_screen:
                    ...
                    self.key_actions["right_mouse_clicked"] = False
        if self.key_actions["up_clicked"]:
            self.viewport.y -= 8
        elif self.key_actions["down_clicked"]:
            self.viewport.y += 8
        if self.key_actions["right_clicked"]:
            self.viewport.x += 8
        elif self.key_actions["left_clicked"]:
            self.viewport.x -= 8
        if self.key_actions["left_mouse_clicked"]:
            ...
            if self.selected_node is not None:
                if not self.selected_node.selected_by_drag_selection:
                    if self.start_command_recording == True:
                        self.start_command_recording = False
                        self.previous_commands.append([(self.selected_node, scaled_mouse_pos[0], scaled_mouse_pos[1])])
                        self.next_commands_undone = []
                    self.selected_node.x = scaled_mouse_pos[0]
                    self.selected_node.y = scaled_mouse_pos[1]
                    if len(self.selected_node.split_of_nodes_partners) > 0:
                        for node in self.all_network_nodes:
                            if node in self.selected_node.split_of_nodes_partners:
                                if node.rect.colliderect(self.selected_node.rect):
                                    self.selected_node = self.merge_split_nodes_back_together(self.selected_node, node)
                else:
                    previous_x, previous_y = self.selected_node.x, self.selected_node.y
                    self.selected_node.x = scaled_mouse_pos[0]
                    self.selected_node.y = scaled_mouse_pos[1]
                    distance_x, distance_y = self.selected_node.x - previous_x, self.selected_node.y - previous_y
                    temp_commands = []
                    for node in self.all_network_nodes:
                        if node.selected_by_drag_selection and node != self.selected_node:
                            if self.show_compartments:
                                node.x += distance_x
                                node.y += distance_y
                                node.fixed_by_mouse = True
                    ...
                    if self.start_command_recording == True:
                        temp_commands.append((self.selected_node, scaled_mouse_pos[0], scaled_mouse_pos[1]))
                        self.start_command_recording = False
...
class Application:
    ...
    def create_listboxes_visibility(self):
        ...
        self.names_left_visibility = [node.instance.name if isinstance(node.instance, MetaboliteNodes) else node.instance.id for node in
                                      self.met_network.all_network_nodes if
                                      not isinstance(node.instance, BoundaryNode) and node.visible_by_mouse and node.instance.id != "000fake"]
        self.names_right_visibility = [node.instance.name if isinstance(node.instance, MetaboliteNodes) else node.instance.id for node in
                                       self.met_network.all_network_nodes if
                                       not isinstance(node.instance, BoundaryNode) and not node.visible_by_mouse and node.instance.id != "000fake"]
        ...
        self.left_list_visibility.bind('<Double-Button-1>',
                                       lambda event: self.on_double_click_visibility(event, self.left_list_visibility, self.right_list_visibility))
        self.right_list_visibility.bind('<Double-Button-1>',
                                        lambda event: self.on_double_click_visibility(event, self.right_list_visibility, self.left_list_visibility))
